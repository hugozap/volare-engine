...
impl ConstraintSystem {
    ...
    pub fn add_entity(&mut self, id: String) -> anyhow::Result<()> {
        ...
        self.solver
            .add_edit_variable(vars.width, WEAK)
            .map_err(|e| anyhow::anyhow!("Failed to add width edit variable: {:?}", e))?;
        self.solver
            .add_edit_variable(vars.height, WEAK)
            .map_err(|e| anyhow::anyhow!("Failed to add height edit variable: {:?}", e))?;
        self.solver
            .add_edit_variable(vars.x, WEAK)
            .map_err(|e| anyhow::anyhow!("Failed to add x edit variable: {:?}", e))?;
        self.solver
            .add_edit_variable(vars.y, WEAK)
            .map_err(|e| anyhow::anyhow!("Failed to add y edit variable: {:?}", e))?;
    ...
    pub fn add_constraint(&mut self, constraint: SimpleConstraint) -> Result<()> {
        match constraint {
            SimpleConstraint::AlignLeft(id1, id2) => {
                ...
                self.solver.add_constraint(vars1.x | EQ(REQUIRED) | vars2.x);
            }
        }
        Ok(())
    }
    ...
    pub fn suggest_size(
        ...
        if let Some(vars) = self.variables.get(id) {
            self.solver.suggest_value(vars.width, width as f64)?;
            self.solver.suggest_value(vars.height, height as f64)?;
        }
    ...
    pub fn suggest_position(
        ...
        if let Some(vars) = self.variables.get(id) {
            self.solver.suggest_value(vars.x, x as f64)?;
            self.solver.suggest_value(vars.y, y as f64)?;
        }
        Ok(())
    }
}
